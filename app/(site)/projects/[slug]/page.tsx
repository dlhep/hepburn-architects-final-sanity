...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    ...
    {project.gallery && project.gallery.length > 0 && <section className={`section project-gallery-section${project.showFullImage ? ` ${imageStyles.fullImageGallery}` : ""}`}><div className={`shell project-gallery${project.gallery.length === 1 ? " project-gallery-single" : ""}`}>{project.gallery.map((image, index) => <figure key={image.asset?._id || index}><div className="project-gallery-media"><Image src={projectImageUrl(image, 1400)} alt={image.alt || `${project.title} project image ${index + 1}`} fill sizes={(project.gallery?.length ?? 0) === 1 ? "(max-width: 950px) 100vw, 1100px" : "(max-width: 950px) 100vw, 50vw"} /></div>{image.caption && <figcaption>{image.caption}</figcaption>}</figure>)}</div></section>}
