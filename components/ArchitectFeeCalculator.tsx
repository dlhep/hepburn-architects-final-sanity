...
export function ArchitectFeeCalculator(){
  ...
  return <div className="fee-tool">
    <div className="fee-form">
      ...
      {current.metric==="area" && <label>Approximate project floor area<div className="area-value">{area} m²</div><input type="range" min="10" max="300" step="5" value={area} onChange={e=>setArea(Number(e.target.value))}/></label>}
      {current.metric==="units" && <label>Number of proposed homes / flats<div className="area-value">{units}</div><input type="range" min="1" max="12" value={units} onChange={e=>setUnits(Number(e.target.value))}/></label>}
      {current.metric==="bedrooms" && <label>Proposed HMO bedrooms<div className="area-value">{bedrooms}</div><input type="range" min="3" max="12" value={bedrooms} onChange={e=>setBedrooms(Number(e.target.value))}/></label>}
