...
const filterMap: Array<[string, string[]]> = [["House Extensions", ["House extension"]], ["Planning Applications", ["Planning application"]], ["Building Regulations", ["Building Regulations"]], ["New Homes", ["New-build home"]], ["Loft Conversions", ["Loft conversion"]], ["HMOs and Conversions", ["HMO conversion", "Change of use"]], ["Developments", ["Residential development", "Commercial project"]]];
...
export function ReviewsCollection({ reviews, featuredIds = [] }: { reviews: Review[]; featuredIds?: string[] }) {
  ...
  const filters = filterMap.filter(([, types]) => reviews.filter((review) => types.includes(review.projectType || "")).length >= 2);
