...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const firstGalleryDimensions = project.gallery?.[0]?.asset?.metadata?.dimensions;
  const portraitLeadGallery = project.showFullImage && project.gallery?.length === 4 && firstGalleryDimensions?.width && firstGalleryDimensions?.height && firstGalleryDimensions.height > firstGalleryDimensions.width && project.gallery.slice(1).every((image) => {
  ...
  return <>
    ...
    <section className="section project-content-section"><div className="shell">
      ...
      <div className="project-detail-grid"><div><small className="eyebrow">Project overview</small><h2>{project.projectType}</h2><p className="lead">{project.description}</p></div>
        <dl className="project-facts">{project.isConcept && <div><dt>Stage</dt><dd>{project.conceptLabel || "Concept study"}</dd></div>}<div><dt>Location</dt><dd>{project.location}</dd></div>{project.localAuthority && <div><dt>Local authority</dt><dd>{project.localAuthority}</dd></div>}{project.applicationType && <div><dt>Application</dt><dd>{project.applicationType}</dd></div>}{project.contractValue && <div><dt>Indicative value</dt><dd>{project.contractValue}</dd></div>}{project.completion && <div><dt>Completion</dt><dd>{project.completion}</dd></div>}</dl>
      </div>
    ...
    {project.gallery && project.gallery.length > 0 && <section className={`section project-gallery-section${project.showFullImage ? ` ${imageStyles.fullImageGallery}` : ""}`}><div className={`shell project-gallery${project.gallery.length === 1 ? " project-gallery-single" : ""}${portraitLeadGallery ? ` ${imageStyles.portraitLeadGallery}` : ""}`}>{project.gallery.map((image, index) => <figure key={image.asset?._id || index}><div className="project-gallery-media" style={image.asset?.metadata?.dimensions?.width && image.asset?.metadata?.dimensions?.height ? { "--project-image-aspect": `${image.asset.metadata.dimensions.width} / ${image.asset.metadata.dimensions.height}` } as CSSProperties : undefined}><Image src={projectImageUrl(image, 1400)} alt={image.alt || `${project.title} project image ${index + 1}`} fill sizes={(project.gallery?.length ?? 0) === 1 ? "(max-width: 950px) 100vw, 1100px" : "(max-width: 950px) 100vw, 50vw"} /></div>{image.caption && <figcaption>{image.caption}</figcaption>}</figure>)}</div></section>}
    <section className="section sand-section"><div className="shell service-deliverables"><div><small className="eyebrow">Architectural services</small><h2>{project.isConcept ? "Design focus." : "Support provided by Hepburn Architects."}</h2></div><div className="deliverables-grid">{(project.services ?? []).map((service) => <div key={service}><CheckCircle2 /> {service}</div>)}</div></div></section>
